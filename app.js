...
function loadParticipants() {
  return fetch(apiUrl)
    .then((response) => {
      if (!response.ok) {
        throw new Error(
          `Failed to Fetch Participants Data! ${response.status}`
        );
      }
...
function updateParticipantCounts() {
  ...
  participantElements.forEach((element) => {
    ...
    const programData = JSON.parse(
      decodeURIComponent(programCard.dataset.program)
    );
...
function getParticipantsByName(programName) {
  ...
  const program = participants.find(
    (item) => item.name.toLowerCase() === programName.toLowerCase()
  );
  ...
  try {
    ...
    programs = Object.fromEntries(
      Object.entries(programs).filter(
        ([_, programsList]) => programsList.length > 0
      )
    );
  } catch (error) {
...
function createProgramCard(program) {
  ...
  const opensClass =
    program.opens && new Date() < new Date(program.opens) ? "opens-soon" : "";
  ...
  const participantsText =
    program.participants !== undefined
      ? `<div class="program-participants">${formatParticipants(
          program.name
        )}</div>`
      : "";

  return `
        <div class="card program-card ${opensClass}" data-program="${encodedProgram}" data-name="${
    program.name
  }">
            <div class="program-header">
                <h3>${program.name}</h3>
                <div class="status-container">
                    <span class="user-completed-badge ${
                      isCompletedByUser ? "visible" : ""
                    }">
    ...
    program.status
            ...
            <div class="program-footer">
                <div class="program-links">
                    ...
                    ${
                      program.slack
                        ? `<a href="${program.slack}" target="_blank">${program.slackChannel}</a>`
                        : ""
                    }
                </div>
                <button class="program-completion-toggle ${completionButtonClass}" aria-label="${
    isCompletedByUser ? "Mark as not completed" : "Mark as completed"
...
function updatePositionIndicator() {
  ...
  if (visiblePrograms.length > 0) {
    positionElement.textContent = `${currentProgramIndex + 1} of ${
...
function openModal(program) {
  ...
  modal.querySelector(
    ".program-status"
  ).className = `program-status status-${program.status}`;
  ...
  const defaultSteps = [
    program.website
      ? `Visit the <a href="${program.website}" target="_blank">program website</a>`
      : null,
    program.slack
      ? `Join the discussion in <a href="${program.slack}" target="_blank">${program.slackChannel}</a>`
      : null,
  ].filter(Boolean);
  ...
  const links = [];
  if (program.website)
    links.push(`<a href="${program.website}" target="_blank">Website</a>`);
  if (program.slack)
    links.push(
      `<a href="${program.slack}" target="_blank">${program.slackChannel}</a>`
    );
...
function renderPrograms() {
  ...
  if (currentSort === "default") {
    for (const [category, programsList] of Object.entries(programs)) {
      ...
      section.innerHTML = `
                <h2 class="headline">${category
                  .replace(/([A-Z])/g, " $1")
                  .replace(/^./, (str) => str.toUpperCase())}</h2>
                <div class="programs-grid">
                    ${programsList
                      .map((program) => createProgramCard(program))
                      .join("")}
                </div>
            `;
...
function filterPrograms(category) {
  ...
  document
    .getElementById("user-not-completed-empty")
  ...
  sections.forEach((section) => {
    ...
    const hasVisibleCards = Array.from(programCards).some(
      (card) =>
        !card.classList.contains("hidden-by-filter") &&
        !card.classList.contains("hidden-by-search")
    );
  ...
  if (category === "user-completed" || category === "user-not-completed") {
    ...
    const hasVisibleCards = Array.from(allProgramCards).some(
      (card) =>
        !card.classList.contains("hidden-by-filter") &&
        !card.classList.contains("hidden-by-search")
    );
    ...
    if (!hasVisibleCards) {
      if (category === "user-completed") {
        document
          .getElementById("user-completed-empty")
...
function searchPrograms(searchTerm) {
  ...
  programCards.forEach((card) => {
    ...
    const slackChannel =
      card.querySelector(".program-links")?.textContent.toLowerCase() || "";
